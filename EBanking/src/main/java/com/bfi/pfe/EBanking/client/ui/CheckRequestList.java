...
public class CheckRequestList extends VLayout {
	private final EbankingServiceAsync ebankingService = GWT
			.create(EbankingService.class);
	private CheckListRequestGrid creditCardListGrid = null;
	private UserDTO user;

	public CheckRequestList(UserDTO user) {
		creditCardListGrid = new CheckListRequestGrid();
					...
					public void onSuccess(List<AccountDTO> result) {
						...
						if (result != null && !result.isEmpty()) {
							for (AccountDTO account : result) {
								ebankingService.getListofCheckRequestByAccountId(
										account.getId(),
										new AsyncCallback<List<CheckRequestDTO>>() {
											...
											public void onSuccess(
													List<CheckRequestDTO> result) {
												for (CheckRequestDTO card : result) {
													// TODO Auto-generated
													// method stub
